	  ...
	  private void spawnRandom() {//pick a spot and a side where 2 or 4 is gonna be 
		...
		while(notValid) {
		   ...
		   if (current==null){
			...
			Bar bar = new Bar(value,getBarX(col),getBarY(row));
...
class GameOver extends JFrame implements ActionListener {
	private JLabel lbl1;
	private Font endGame = new Font("Calibri", Font.PLAIN, 14);   
	  GameOver() {
		  ...
		  setResizable(false);
		  setTitle("Game Over!");
		  lbl1 = new JLabel("Вы проиграли со счетом " + MyGameBoard.score+"Ваше имя"+ MyGameBoard.namePlayer, SwingConstants.CENTER);
		  JTextField namePlayer= new JTextField();
		  
		  lbl1.setForeground(Color.red);
		  lbl1.setFont(endGame);
		  add(lbl1, BorderLayout.CENTER);
		  add(namePlayer, BorderLayout.SOUTH);
  ...
  class GameWon extends GameOver {
	private JLabel lbl1;
	private Font endGame = new Font("Calibri", Font.BOLD, 15);   
	GameWon() {  
		super();
		setTitle("ПОБЕДА!");
		lbl1 = new JLabel("Вы выиграли! Поздравляем!", SwingConstants.CENTER);
		lbl1.setForeground(Color.green);
		lbl1.setFont(endGame);
		add(lbl1, BorderLayout.CENTER);
	  }
  }
  class YouBetRecord extends GameOver {
		private JLabel lbl1;
		private Font endGame = new Font("Calibri", Font.BOLD, 15);  
		YouBetRecord() {
			super();
			setTitle("Новый рекорд!");
			lbl1 = new JLabel("Вы побили рекорд!", SwingConstants.CENTER);
